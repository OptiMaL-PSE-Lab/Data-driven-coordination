...
def centralized_nonconv(data, dim=None):
    if dim is None:
    ...
    def o(m):
        ...
        return sum(m.x[n]*cost[n-1] + \
                   m.x[n]**powers[a[n-1]]*df_env['CC (kg CO2-eq)'][n-1] + \
...
def centralized_conv(data, dim=None):
    if dim is None:
        raise ValueError('dim is None')
    ...
    def o(m):
        return sum(m.x[n]*cost[n-1] + \
                   m.x[n]**powers[a[n-1]]*df_env['CC (kg CO2-eq)'][n-1]  for n in m.I)
    m.obj = pyo.Objective(rule = o)
